...
OTHER_NODES_EMPTY=$(sed -n '/othernodes/p' VANode.conf)
...
mv qdata/VANode.mv.db qdata/VANode.mv.db.bak

sed -i "s|h2url|jdbc:h2:file:/home/node/qdata/VANode;AUTO_SERVER=TRUE|" qdata/tessera-migration.properties

${tessera_data_migration} -storetype dir -inputpath qdata/storage/payloads -dbuser -dbpass -outputfile qdata/VANode -exporttype JDBC -dbconfig node/tessera-migration.properties >> /dev/null 2>&1

if [ ! -f qdata/VANode.mv.db ]; then
    mv qdata/VANode.mv.db.bak qdata/VANode.mv.db
fi

${tessera_config_migration} --tomlfile="VANode.conf" --outputfile tessera-config.json --workdir= " >> /dev/null 2>&1

sed -i "s|jdbc:h2:mem:tessera|jdbc:h2:file:/home/node/qdata/VANode;AUTO_SERVER=TRUE|" tessera-config.json
sed -i "s|/home/node/qdata/home|/home|" tessera-config.json
sed -i "s|/.*.ipc|/home/node/qdata/VANode.ipc|" tessera-config.json
...
sed -i "s|Starting Constellation node|Starting Tessera node|" start_VANode.sh
sed -i "s|qdata/constellationLogs/constellation_|qdata/tesseraLogs/tessera_|" start_VANode.sh
sed -i "s|constellation-node.*conf|\$tessera -configfile tessera-config.json|" start_VANode.sh
